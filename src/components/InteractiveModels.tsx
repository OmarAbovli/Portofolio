...
const DNA = ({ mousePosition }: { mousePosition: { x: number; y: number } }) => {
  ...
  useFrame((state) => {
    if (groupRef.current) {
      ...
      groupRef.current.position.x = mousePosition.x * 1.5;
      groupRef.current.position.y = mousePosition.y * 0.8;
    }
  });
...
const MorphingCrystal = ({ mousePosition }: { mousePosition: { x: number; y: number } }) => {
  ...
  useFrame((state) => {
    if (meshRef.current) {
      ...
      // Floating animation
      meshRef.current.position.y = 1 + mousePosition.y * 0.6 + Math.sin(state.clock.elapsedTime * 1.5) * 0.4;
      meshRef.current.position.x = -3 + mousePosition.x * 1.2;
    }
  });
  ...
  useFrame((state) => {
    if (textRef.current) {
      ...
      if (textRef.current.material && 'opacity' in textRef.current.material) {
        (textRef.current.material as any).opacity = 0.7 + Math.sin(state.clock.elapsedTime * 4) * 0.3;
      }
    }
  });
